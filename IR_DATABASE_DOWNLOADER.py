...
cur=conn.cursor()   
fg=[]#FUNCTIONAL GROUP STORAGE 
vib=['all','stretching', 'bending']#TYPE OF VIBRATION
vib_type='all'
func_type='all'
cm=0
dat_check=0
cmdlineq=''
statement='ENTER INFORMATION'
list_item=''
def func_group():
    global fg
            ...
            if(len(wavenumber_range)==1):
                ...
                for k in range(len(wavenumber_range)):
                    ...
                    if row is None:
                        ...
                        cur.execute(command3, (wavenumber_upper,wavenumber_lower,intensity,peak,functional_group,type_of_vibration,group_name,other))
        conn.commit()
    messagebox.showinfo("STATUS", "DATABASE UPDATED SUCCESSFULLY") 
...
def interface_user():
    ...
    infoview=Frame(INFO)
    infoview.pack()
    lbl5 = Label(infoview, text="CHEMICAL ENGINEERING TOOLKIT", font=("Arial Bold", 14))
    lbl5.pack(fill=X)
    ...
    lble1.pack(fill=X)
    def help_me():
        messagebox.showinfo("GUIDELINES", "Enter the infrared spectrum wavenumber. Then select Estimate Functional Group button to show the corresponding matching functional groups. You can also limit the chemical group and type of vibration by choosing the options from the listbox. Once in a year or two, you can update the database for any new compounds") 
        # lbl4 = Label(infoview, text="Guidelines", font=("Arial Bold", 12))
        # lbl4.pack(fill=X)
        # info1 = Message(infoview, text = "Enter the infrared spectrum wavenumber. Then select Estimate Functional Group button to show the corresponding matching functional groups. You can also limit the chemical group and type of vibration by choosing the options from the listbox. Once in a year or two, you can update the database for any new compounds")
        # info1.config(font=('Arial', 12), justify=CENTER, width=860)
        # info1.pack(fill=X)
    #lbupdate=Label(finalshowdown, text="DATABASE UPDATED", font=("Arial", 14)).pack(side=TOP, anchor=NW)
    enteroption = Frame(IR_ESTIMATE)
    enteroption.grid(row=0,column=0, sticky=NW)
    optlabel=Label(enteroption, text="IR wavenumber (per cm)", justify='center',font=("Arial", 13,"bold"))
    optlabel.pack(side=TOP)
    ...
    typefg=Frame(IR_ESTIMATE)
    typefg.grid(row=0,column=1)
    lb1=Label(typefg, text="Choose Chemical Group",justify='center', font=("Arial", 13,"bold")).pack(side=TOP)
    option2=Entry(typefg, width=23,font=("Arial", 12))
    option2.pack(side=TOP, anchor=NW,fill="x")
    search=Button(typefg, text="Search", font=("Arial", 13,"bold"), height=1, width=23)
    search.pack(side=TOP,anchor=N,fill='x',expand=TRUE)
    
    listfunc = Listbox(typefg, width=40, height=5,justify='center', font=("Helvetica", 12),selectmode=BROWSE,exportselection=False)
    listfunc.pack(side=LEFT, fill="x")
    scrollbar = Scrollbar(typefg, orient="vertical")
    scrollbar.config(command=listfunc.yview)
    scrollbar.pack(side=RIGHT,anchor=N, fill="y")
    lbdisp=ScrolledText(ESTIMATE,width=300, height=10,font=("Arial", 12))
    lbdisp.pack(side=LEFT,fill='none')      
    lbdisp.configure(state='disabled')
    while cm!=1:
        func_group() 
        for items in range(len(vib)):
            listvib.insert(items, vib[items])
        for items in range(len(fg)):
            listfunc.insert(items, fg[items])
        cm=1   
      
    def vib_type_choosen(evt):
        global vib_type
        vib_type=str((listvib.get(ANCHOR)))
    def func_type_choosen(evt):
        global func_type
        func_type=str((listfunc.get(ANCHOR)))
    def solvefunc():
        ...
        global cmdlineq
        dat_check=0
        if(vib_type=='all' and func_type=='all' or func_type==''):
            cmdlineq="SELECT * FROM IR_DATA WHERE wavenumber_upper>="+str(x)+" AND wavenumber_lower<="+str(x)
        elif(vib_type!='all' and func_type=='all'):
            cmdlineq="SELECT * FROM IR_DATA WHERE type_of_vibration='"+str(vib_type)+"' AND wavenumber_upper>="+str(x)+" AND wavenumber_lower<="+str(x)
        elif(vib_type=='all' and func_type!='all'):
            cmdlineq="SELECT * FROM IR_DATA WHERE group_name='"+str(func_type)+"' AND wavenumber_upper>="+str(x)+" AND wavenumber_lower<="+str(x)
        elif(vib_type!='all' and func_type!='all'):
            cmdlineq="SELECT * FROM IR_DATA WHERE type_of_vibration='"+str(vib_type)+"' AND group_name='"+str(func_type)+"' AND wavenumber_upper>="+str(x)+" AND wavenumber_lower<="+str(x) 
    ...
    def new_list_find(event):
        listfunc.delete('0','end')
        global list_item
        global fg
        list_item=option2.get()
        indices = [i for i, s in enumerate(fg) if list_item in s]
        new_list=list()
        for i in indices:
            new_list.append(fg[i])
        for items in range(len(new_list)):
            listfunc.insert(items, new_list[items])
    option2.bind('<Key-Return>',new_list_find)
    listvib.bind('<<ListboxSelect>>',vib_type_choosen)
    listfunc.bind('<<ListboxSelect>>',func_type_choosen)
    search.bind('<Button-1>',new_list_find)
    window.mainloop()
